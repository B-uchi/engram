...
# Token counting — use cl100k_base (close enough for Qwen3 tokenizer)
...
class MemoryStore:
    ...
    async def update_memory(self, memory_id: str, content: str, reason: str) -> Memory:
        """Update memory content (used by consolidator for non-contradiction corrections)."""
        async with self._session() as db:
    ...
    async def deprecate_memory(
        ...
        """Deprecate a memory — keep it for audit, remove from active retrieval."""
        async with self._session() as db:
            result = await db.execute(select(Memory).where(Memory.id == memory_id))
            memory = result.scalar_one_or_none()
            if not memory:
    ...
    async def query_memory(
        ...
        for m in candidates:
            ...
            recency_scores[m.id] = math.exp(-0.05 * age_days)  # half-life ~14 days
